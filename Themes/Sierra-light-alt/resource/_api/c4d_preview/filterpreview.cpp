...
inline Float32 BLEND_WHITE(Float32 v)	{ return ((v * 4) - v + 0xffL / 255.0) / 4; }
inline Float32 BLEND_BLACK(Float32 v) { return ((v * 4) - v + 0x80L / 255.0) / 4; }

template <class T> void blend_pattern(BM_TILE* tile, T dummy)
{
#define	PAT_WIDTH	 8
#define	PAT_HEIGHT 8
...
PreviewBmp::PreviewBmp(void)
{
	...
	offset_y	 = 0;
	real_time	 = false;
	down_scale = prefs.GetInt32(BF_PREFS_PREVIEW_PERCENT, SCALE_ONE);	// ###NOTE: hier ist noch Gefrickel zwischen PreviewBmp und PreviewGroup. Bei Gelegenheit aufrŠumen
...
void PreviewGroup::AddPreview(GeDialog* dlg, Int32 w, Int32 h, PreviewBmp& preview, Int32 preview_id)
{
	dlg->GroupBegin(0, BFV_CENTER, 1, 0, String(), 0);
	{
	...
	dlg->AttachUserArea(preview, preview_id, USERAREA_COREMESSAGE | USERAREA_DONT_MIRROR);
}
