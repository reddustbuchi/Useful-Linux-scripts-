...
Bool RegisterPrefsPlugin(Int32 id, const String &str, Int32 info, DataAllocator *npalloc, const String &description, BaseBitmap *icon)
{
...
#ifdef __API_INTERN__
	BaseContainer *GetWorldContainerInstance() { return &world->prefs; }
#endif
...
void PrefsDialogObject::InitPrefsValue(Int32 id,const GeData &data, Description* desc,const DescID &descid, BaseContainer* pBC)
{
	...
	else
	{
		...
		{
			//Tilo oder Philip ist das so richtig??
#ifdef __API_INTERN__
			BaseContainer temp1;
			AtomArray ar;
