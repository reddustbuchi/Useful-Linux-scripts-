...
const VERSION = 'bdftofnt 1.48, Copyright (C) 2018 Dimitar Toshkov Zhekov\n\n' + fnutil.GPL2PLUS_LICENSE;
...
class Options extends fncli.Options {
	...
	parse(name, optarg) {
		switch (name) {
		case '-c':
			this.params.charSet = fnutil.parseDec('charset', optarg, 0, 255);
			break;
		case '-m':
			this.params.minChar = fnutil.parseDec('minchar', optarg, 0, 255);
			break;
		case '-f':
			this.params.fntFamily = FNT_FAMILIES.indexOf(optarg);

			if (this.params.fntFamily === -1) {
...
function mainProgram(nonopt, parsed) {
	...
	try {
		...
		ifs.close();
		font.checkBold(ifs.stName);
		font.checkItalic(ifs.stName);
	} catch (e) {
	...
	try {
		...
		ofs.write16(Math.round(font.bbx.height * 72 / 96));
		...
		// GLYPHS
		let data = new Buffer(font.bbx.height * font.bbx.rowSize());
...
fncli.start('bdftofnt.js', new Options(), mainProgram);
