...
class Char {
	...
	write(ofs, maxWidth, yoffset) {
		let header = `STARTCHAR ${this.name}\nENCODING ${this.code}\n`;
		const swidth = Math.round(this.width * 1000 / maxWidth);
		const height = this.data.length / this.rowSize();

		header += `SWIDTH ${swidth} 0\nDWIDTH ${this.width} 0\nBBX ${this.width} ${height} 0 ${yoffset}\n`;
		ofs.writeLine(header + 'BITMAP\n' + bdf.Char.bitmap(this.data, this.rowSize()) + 'ENDCHAR');
	}
}
...
class Font extends bdf.Font {
	...
	_read(input) {
		...
		this.avgWidth = Math.round(totalWidth / this.chars.length);
	...
	write(ofs) {
		...
		this.chars.forEach(char => char.write(ofs, this.bbx.width, this.bbx.yoff));
